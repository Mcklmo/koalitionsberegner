...
def saved(store, page=PAGE, election_hash=HASH, election=None):
...
def test_a_page_that_already_produced_an_election_ends_all_claiming(store):
    saved(store)
...
def test_confirming_stores_the_election_and_indexes_the_page(store):
...
def test_a_second_page_describing_the_same_election_does_not_duplicate_it(store):
...
def test_linking_points_a_page_at_an_existing_election(store):
    saved(store, page="page-a")
...
def test_discarding_removes_the_draft_and_frees_the_page(store):
    store.claim(PAGE, make_request())
    store.stage(PAGE, make_election())
...
def test_failing_a_job_records_the_error_and_frees_the_page(store):
    store.claim(PAGE, make_request())
