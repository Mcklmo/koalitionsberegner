...
async def request_election(
    ...
    # Only a *confirmed* address is named in the issue. An unconfirmed one was
    # merely typed by somebody, and publishing it would put a stranger's address
    # on a public issue they never made — so those requests are filed
    # anonymously, exactly as a signed-out one is.
    requester = principal.email if principal and principal.email_verified else None

    try:
        filed = await wishlist.file(request, requester=requester)
    ...
    log.info(
        ...
        principal.uid[:12] if principal else "anonymous",
        filed.number,
